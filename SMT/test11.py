...
def random_selection(res):
    ...
    for idx in index:
        ...
        for strategy_item in strategy_list:
            result.append('./vampire_rel_master_4055' + ' ' + '--input_syntax smtlib2' + ' ' + path + '/' + res[idx] + ' ' + '--include ../SMT/ --decode' + ' ' + strategy_item + ' ' + '--time_statistics on -p off -t 10s ' + '\n')
        result.append('\n')
    index.clear()
    result.append('\n')
    return result
